"""
LTCOE Campus — Flask Backend (Enhanced)
flask
flask-cors
gunicorn
Run: python app.py
...
def add_announcement():
    ...
    db = get_db()
    db.execute("""
        INSERT INTO announcements (title, body, category, posted_by, created_at, priority)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (
        data.get('title', ''),
        data.get('body', ''),
...
def get_timetable():
    db = get_db()
    branch = request.args.get('branch', 'CE')
    year = request.args.get('year', 'SE')
    rows = db.execute(
        "SELECT * FROM timetable WHERE branch=? AND year=? ORDER BY CASE day "
        "WHEN 'Monday' THEN 1 WHEN 'Tuesday' THEN 2 WHEN 'Wednesday' THEN 3 "
        "WHEN 'Thursday' THEN 4 WHEN 'Friday' THEN 5 WHEN 'Saturday' THEN 6 END, time_slot",
        (branch, year)
    ).fetchall()
    return jsonify(rows_to_list(rows))
...
def add_feedback():
    ...
    db = get_db()
    db.execute("""
        INSERT INTO feedback (student_id, student_name, subject, rating, comment, created_at)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (
...
def get_faculty():
    db = get_db()
    rows = db.execute("SELECT id, email, name, initials, department, phone FROM users WHERE role='Faculty' ORDER BY name").fetchall()
    return jsonify(rows_to_list(rows))
...
def stats():
    ...
    present = db.execute("SELECT COUNT(*) FROM attendance_log WHERE date=? AND status='P'", (today_d,)).fetchone()[0]
